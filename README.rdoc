...
Once the gem is installed, you can start a Rails console and try the following:
    ...
    $ job.add_function("blur", {radius:1}) # Add a blur function, with no params, and give a unique name to the result
    $ job.add_save("my_first_image") # Adds a save function (which identifies what the image is identified as on callback/polling
